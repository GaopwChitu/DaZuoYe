  ...
  <div>
    <el-card id="search">
      <el-row>
        <el-col :span="23">
          ...
          <el-input v-model="searchModel.sportType" placeholder="运动类型" clearable></el-input>
          <el-button @click="getDetailList" type="primary" round icon="el-icon-search">查询</el-button>
        </el-col>
        <el-col :span="1">
    ...
    <el-card>
      <el-table :data="detailList" stripe style="width: 100%">
        ...
        <el-table-column prop="sportType" label="运动类型" width="180"></el-table-column>
        <el-table-column prop="disease" label="禁忌疾病" width="180"></el-table-column>

        <el-table-column prop="method" label="运动方法" width="180"></el-table-column>

        <el-table-column prop="notes" label="注意事项" width="180"></el-table-column>

        <el-table-column label="操作" width="180">
          <!-- 删除和修改按钮 -->
          <template slot-scope="scope">
            <el-button @click="openEditUi(scope.row.id)" type="primary" icon="el-icon-edit" circle></el-button>
            <el-button @click="deleteDetail(scope.row)" type="danger" icon="el-icon-delete" circle></el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    <el-dialog @close="clearForm" :title="title" :visible.sync="dialogFormVisible">
      <el-form :model="detailForm" ref="detailFormRef">
        ...
        <el-form-item label="禁忌疾病" prop="disease" :label-width="formLabelWidth">
        ...
        <el-form-item label="注意事项" prop="notes" :label-width="formLabelWidth">
...
export default {
  ...
  methods: {
    ...
    saveDetailForm(detail) {
      if (detail.id == null && detail.id == undefined) {
        return this.addDetail(detail);
      }
      return this.updateDetail(detail);
    },

    async addDetail(detail) {
      try {
        const res = await this.$http.post("detail/addDetail", detail);
        return res.data;
      } catch (error) {
        console.log("新增DetailForm错误");
      }
    },
    async updateDetail(detail) {
      const res = await this.$http.put("detail/Detail", detail);
      return res.data;
    },
    saveDetail() {
      let isOk = true;
      ...
      if (isOk) {
        ...
        this.saveDetailForm(this.detailForm).then((response) => {
          //成功提示
          this.$message({
    ...
    async getDetailFormList(searchModel) {
      const res = await this.$http.get("detail/getDetailList", {
        params: {
    ...
    //用于查询用户列表
    getDetailList() {
      this.getDetailFormList(this.searchModel).then((response) => {
    ...
    async getDetailById(id) {
      const res = await this.$http.get(`detail/getDetailById/${id}`)
      return res.data;
    },
    openEditUi(id) {
      if (id == null) {
        ...
        this.title = "修改运动详情";
        //根据id查询用户数据
        this.getDetailById(id).then((response) => {
    ...
    async deleteDetailById(id) {
      const res = await this.$http.delete(`detail/deleteDetailById/${id}`)
      return res.data;
    },
    deleteDetail(detail) {
      this.$confirm(`确认删除 ${detail.sportType} 这个运动详情吗？`, "提示", {
        ...
        .then(() => {
          this.deleteDetailById(detail.id).then((response) => {
            this.$message({
              type: "success",
              message: response.message,
            });
