  ...
  <div>
    ...
    <el-card style="margin-right: 20px;margin-bottom: 20px">
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <label>医院名称：</label>
          <el-input placeholder="请输入医院名称" v-model="queryInfo.hospitalName" clearable
            style="width: 70%; padding-right: 10px" />
        </el-col>
        <el-col :span="8">
          <label>地址：</label>
          <el-input placeholder="请输入医院地址" v-model="queryInfo.address" clearable
            style="width: 70%; padding-right: 10px" />
        </el-col>
        ...
        <el-col :span="8">
          ...
          <el-button type="primary" icon="el-icon-plus" @click="isAddHospital = true"
            v-show="this.role == '管理员'">添加医院</el-button>
        </el-col>
      </el-row>
    </el-card>
    ...
    <!--    添加医院-->
    <el-dialog title="添加医院" :visible.sync="isAddHospital" width="50%" @close="addHospitalClosed">
      <el-form :model="addHospitalForm" :rules="addHospitalFormRules" ref="addHospitalFormRef" label-width="160px">
        ...
        <el-form-item label="医院放号时间：" prop="registerTime">
          <el-time-select v-model="addHospitalForm.registerTime"
            :picker-options="{ start: '06:00', step: '00:15', end: '18:30' }" placeholder="选择时间">
          </el-time-select>
        </el-form-item>
      </el-form>
      <!-- 内容底部区域    -->
    ...
    <el-dialog title="预约挂号" :visible.sync="isAppoint" width="50%" @close="addAppointClosed">
      <el-form :model="addAppointForm" :rules="addHospitalFormRules" ref="addAppointFormRef" label-width="160px">
        ...
        <el-form-item label="科室:" prop="departmentId">
          <el-select v-model="addAppointForm.departmentId" clearable filterable placeholder="请选择"
            @change="getDoctorDrop(addAppointForm.departmentId)">
        ...
        <el-form-item label="预约时间：" prop="registerTime">
          <el-date-picker v-model="addAppointForm.weekdays" type="date" placeholder="选择日期时间"
            :picker-options="pickerOptions">
          </el-date-picker>
          <el-time-select v-model="addAppointForm.registerTime"
            :picker-options="{ start: addAppointForm.registerTime, step: '00:15', end: '18:30' }" placeholder="选择时间">
          </el-time-select>
        </el-form-item>
  ...
  methods: {
    ...
    onClickAppoint() {
      this.$refs.addAppointFormRef.validate(async valid => {
        ...
        this.addAppointForm.createId = (JSON.parse(window.sessionStorage.getItem("user"))).id
    ...
    onClickOk() {
      this.$refs.addHospitalFormRef.validate(async valid => {
        if (!valid) return;
        this.addHospitalForm.createId = (JSON.parse(window.sessionStorage.getItem("user"))).id
    ...
    handleAvatarSuccess(res, file) {
      ...
      this.addHospitalForm.logo = 'assets/imgs/' + newFile.get("imgUrl")
