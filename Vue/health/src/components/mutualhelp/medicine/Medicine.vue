  ...
  <div>
    ...
    <el-card>
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <label>用户姓名：</label>
          <el-input placeholder="请输入用户姓名" v-model="queryInfo.username" clearable
            style="width: 70%; padding-right: 10px" />
        </el-col>
        <el-col :span="8">
          <label>业务类型：</label>
          <el-select v-model="queryInfo.businessType" placeholder="请选择业务类型" clearable filterable
            style="width: 70%; padding-right: 10px">
            <el-option v-for="item in options" :key="item.businessType" :label="item.text" :value="item.businessType">
            </el-option>
          </el-select>
        </el-col>
        <el-col :span="8">
          <label>业务机构：</label>
          <el-input placeholder="请输入业务机构" v-model="queryInfo.businessOrganization" clearable
            style="width: 70%; padding-right: 10px" />
        </el-col>
      </el-row>
      <el-row :gutter="20" style="margin-top: 15px">
        <el-col :span="8">
          <label>业务状态：</label>
          <el-select v-model="queryInfo.businessStatus" placeholder="请选择业务状态" clearable filterable
            style="width: 70%; padding-right: 10px">
            <el-option v-for="item in statusList" :key="item.businessStatus" :label="item.text"
        ...
        <el-col :span="8">
          ...
          <el-button type="warning" icon="el-icon-s-shop" @click="$router.push({ path: '/pharmacy' })">药店</el-button>
          <el-button type="success" icon="el-icon-s-finance"
            @click="$router.push({ path: '/appointment' })">预约挂号</el-button>
        </el-col>
      </el-row>
    ...
    <el-dialog title="业务详情" :visible.sync="isMedical" width="50%" @close="addClosed">
      <el-form :model="medicalInfoForm" :rules="medicalInfoFormRules" ref="medicalInfoFormRef" label-width="160px">
        ...
        <el-form-item label="科室:" prop="departmentId">
          <el-select v-model="medicalInfoForm.departmentId" clearable filterable placeholder="请选择"
            @change="getDoctorDrop(medicalInfoForm.departmentId)">
            <el-option v-for="item in departmentDrop" :key="item.departmentId" :label="item.departmentName"
        ...
        <el-form-item label="医生:" prop="doctorId">
          <el-select v-model="medicalInfoForm.doctorId" clearable filterable placeholder="请选择">
            <el-option v-for="item in doctorDrop" :key="item.doctorId" :label="item.doctor" :value="item.doctorId">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="预约时间：" prop="registerTime">
          <el-date-picker v-model="medicalInfoForm.appointTime" type="datetime" placeholder="选择日期时间">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="门诊费：" prop="address">
          <el-input v-model="medicalInfoForm.price" size="medium" style="width: 70%" disabled><i slot="suffix"
...
export default {
  ...
  methods: {
    async getMedicalList() {
      // 构建请求参数：将搜索条件和角色过滤条件合并
      const params = { ...this.queryInfo };

      if (this.role == '用户') {
        // 用户只能看到自己的预约挂号和买药记录
        params.createId = this.userId;
      } else if (this.role == '管理员') {
        // 管理员能看到所有用户的记录，不额外添加过滤条件
      }

      const { data: res } = await this.$http.get("/medical/list", { params })
      this.medicalList = res
      this.total = this.medicalList[0].total || 0
    ...
    onClickSave() {
      this.$refs.medicalInfoFormRef.validate(async valid => {
        if (!valid) return;
        this.medicalInfoForm.updateId = (JSON.parse(window.sessionStorage.getItem("user"))).id
