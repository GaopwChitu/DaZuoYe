  ...
  <div>
    ...
    <el-card>
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <label>订单编号：</label>
          <el-input placeholder="请输入订单编号" v-model="queryInfo.foodOrderNo" clearable
            style="width: 70%; padding-right: 10px" />
        </el-col>
        <el-col :span="8">
          <el-button type="primary" icon="el-icon-search" @click="search">搜索</el-button>
          <el-button type="warning" icon="el-icon-s-shop" @click="$router.push({ path: '/restaurant' })">餐厅</el-button>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    async deleteFoodOrder(id) {
      ...
      const { data: res } = await this.$http.post("/foodOrder/deleteFoodOrderById?id=" + id);
      if (res !== "success") {
