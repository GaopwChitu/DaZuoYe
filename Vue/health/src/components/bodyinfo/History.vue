    ...
    <el-card>
      <el-table :data="bodyList" stripe style="width: 100%">
        ...
        <el-table-column label="操作" width="180">
          <!-- 删除和修改按钮 -->
          <template slot-scope="scope">
            <el-button @click="openEditUi(scope.row.notesid)" type="primary" icon="el-icon-edit" circle></el-button>
            <el-button @click="deleteUserBody(scope.row)" type="danger" icon="el-icon-delete" circle></el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
...
export default {
  ...
  methods: {
    async updateUserBody(body) {
      const res = await this.$http.put("updateUserBody", body);
      return res.data;
    },
    updateBody() {
      let isOk = true;
      ...
      if (isOk) {
        //提交验证给后台
        this.updateUserBody(this.bodyForm).then((response) => {
          //成功提示
          this.$message({
    ...
    async getUserBodyFormList(searchModel) {
      const user = JSON.parse(window.sessionStorage.getItem("user"));
      const res = await this.$http.get("getUserBodyList", {
        params: {
          userId: user.id,
    ...
    //用于查询用户列表
    getUserBodyList() {
      this.getUserBodyFormList(this.searchModel).then((response) => {
    ...
    async getUserBodyById(notesid) {
      const res = await this.$http.get(`getUserBodyById/${notesid}`)
      return res.data;
    },
    openEditUi(notesid) {
      this.title = "修改身体信息";
      //根据id查询用户数据
      this.getUserBodyById(notesid).then((response) => {
        this.bodyForm = response.data;
      });
      this.dialogFormVisible = true;
    },

    async deleteUserBodyById(id) {
      const user = JSON.parse(window.sessionStorage.getItem("user"));
      const res = await this.$http.delete(`deleteUserBodyById/${id}`, {
        params: {
          userId: user.id,
        }
      })
    ...
    deleteUserBody(body) {
      this.$confirm(`确认删除 ${body.name} 这个身体信息吗？`, "提示", {
        ...
        .then(() => {
          this.deleteUserBodyById(body.notesid).then((response) => {
            this.$message({
              type: "success",
              message: response.message,
            });
