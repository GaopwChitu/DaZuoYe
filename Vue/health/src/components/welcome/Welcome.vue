          ...
          <el-row :gutter="5">
            ...
            <el-col :span="18" class="marquee-wrap">
              <ul class="marquee-list" :class="{ 'animate-up': animateUp }">
                <li v-for="(item, index) in listData" :key="index">{{ item.createTime }} {{ item.username }}: {{
                  item.news }}
                </li>
              </ul>
            </el-col>
...
export default {
  ...
  methods: {
    async getStackedList() {
      this.dataX = []
      const { data: res } = await this.$http.get("/Welcome/stackedList")
      this.stackList = res;
    ...
    async getNumber() {
      const { data: res } = await this.$http.get("/Welcome/number")
    ...
    async submit() {
      ...
      this.newsForm.username = (JSON.parse(window.sessionStorage.getItem("user"))).username;
      const { data: res } = await this.$http.post("/Welcome/news", this.newsForm)
      if (res !== "success") {
    ...
    async getNewsList() {
      const { data: res } = await this.$http.get("/Welcome/list")
      this.listData = res;
