  ...
  <div>
    ...
    <el-card>
      <el-row :gutter="20">
        <el-col :span="8">
          <label>活动名称：</label>
          <el-input placeholder="请输入活动名称" v-model="queryInfo.activeName" clearable
            style="width: 70%; padding-right: 10px" />
        </el-col>
        <el-col :span="8">
          <label>活动区域：</label>
          <el-input placeholder="请输入活动区域" v-model="queryInfo.activeArea" clearable
            style="width: 70%; padding-right: 10px" />
        </el-col>
        <el-col :span="8">
          <el-button type="primary" icon="el-icon-search" @click="search">搜索</el-button>
          <el-button type="success" icon="el-icon-search" @click="searchOwn">已参加</el-button>
          <el-button type="primary" icon="el-icon-plus" @click="isActive = true"
            v-show="this.role == '管理员'">添加活动</el-button>
        </el-col>
      </el-row>
      <el-table :data="activeList" border stripe>
        <el-table-column type="expand">
          <template v-slot="scope">
            <el-form label-position="center" inline class="demo-table-expand">
              <el-form-item label="参赛名单：">
                <el-tag closable v-for="item in scope.row.joinList" :key="item.id" @close="deleteJoins(item)">{{
                  item.joinName }}</el-tag>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="150px">
          <template v-slot="scope">
            <el-button type="success" icon="el-icon-finished" size="mini" @click="joinActive(scope.row.id)">
            </el-button>
            <!--修改-->
            <el-button type="primary" icon="el-icon-edit" size="mini" v-show="role == '管理员'"
              @click="showEditDialog(scope.row.id)">
            </el-button>
            <!--删除-->
            <el-button type="danger" icon="el-icon-delete" size="mini" v-show="role == '管理员'"
              @click="deleteActive(scope.row.id)">
            </el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog title="添加活动" :visible.sync="isActive" width="50%" @close="insertClosed">
        <el-form :model="addActiveForm" :rules="addActiveFormRules" ref="addActiveFormRef" label-width="160px">
          ...
          <el-form-item label="活动时间" prop="activeTime">
            <el-date-picker v-model="addActiveForm.activeTime" type="datetime" placeholder="选择日期时间">
            </el-date-picker>
          </el-form-item>
      ...
      <el-dialog title="活动信息" :visible.sync="isEditActive" width="50%" @close="editClosed">
        <el-form :model="editActiveForm" :rules="addActiveFormRules" ref="editActiveFormRef" label-width="160px">
          ...
          <el-form-item label="活动时间" prop="activeTime">
            <el-date-picker v-model="editActiveForm.activeTime" type="datetime" placeholder="选择日期时间">
            </el-date-picker>
...
export default {
  ...
  methods: {
    ...
    searchOwn() {
      this.queryInfo.joinId = (JSON.parse(window.sessionStorage.getItem("user"))).id
      this.getActiveList()
    ...
    addActive() {
      this.$refs.addActiveFormRef.validate(async valid => {
        if (!valid) return;
        this.addActiveForm.createName = (JSON.parse(window.sessionStorage.getItem("user"))).username
        this.addActiveForm.createId = (JSON.parse(window.sessionStorage.getItem("user"))).id
    ...
    async deleteJoins(item) {
      if ((JSON.parse(window.sessionStorage.getItem("user"))).role !== '管理员') {
        return this.$message.error("权限不够，不能操作！！！")
      }
      if (item.joinId !== (JSON.parse(window.sessionStorage.getItem("user"))).id) {
    ...
    async joinActive(id) {
      ...
      this.joinForm.joinId = (JSON.parse(window.sessionStorage.getItem("user"))).id
      this.joinForm.joinName = (JSON.parse(window.sessionStorage.getItem("user"))).username
    ...
    editActive() {
      this.$refs.editActiveFormRef.validate(async valid => {
        if (!valid) return;
        this.editActiveForm.updateId = (JSON.parse(window.sessionStorage.getItem("user"))).id
